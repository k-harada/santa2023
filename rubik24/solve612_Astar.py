...
def solve_greed_612(initial_state: List[str], goal_state: List[str], two_side: bool = True):
    ...
    while len(open_set_left):
        ...
        if np.random.uniform() < 0.0001:
            ...
            print(_, _current_state, heuristic(current_state, goal_state_arr, two_side))
...
def test_612(initial_state, goal_state):
    path = solve_greed_612(initial_state, goal_state)
    print(path)
    n = 6
...
if __name__ == "__main__":
    ...
    test_612(_initial_state, _goal_state)
    ...
    for _i, _row in puzzles_df_pick.iterrows():
        ...
        _path = solve_greed_612(_initial_state_pick, _goal_state_pick)
        for _m in _path:
            _q6.operate(_m)
        print(len(_path))
        print(_q6.state)

...
from rubik24.compress_magic41 import arr_dict, command_dict
...
def solve_greed_41(initial_state: List[str], goal_state: List[str], two_side: bool = False):
...
def test_41(initial_state, goal_state):
    path = solve_greed_41(initial_state, goal_state)
    print(path)
    n = 4
...
if __name__ == "__main__":
    ...
    for _i, _row in puzzles_df_pick.iterrows():
        ...
        _path = solve_greed_41(_initial_state_pick, _goal_state_pick)
        for _m in _path:
            _q4.operate(_m)
        print(len(_path))
        print(_q4.state)
